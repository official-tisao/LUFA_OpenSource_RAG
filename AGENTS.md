...
# Model proxy — Gemini & Codex CLIs via OpenAI-compatible API (port 9090)
...
- `GET  /v1/models` — list available Gemini/Codex models
- `POST /v1/chat/completions` — OpenAI-compatible chat completion (routes by model prefix via CLI sub-process)
- Requires `Codex` CLI (Codex) and/or `antigravity` (Google Antigravity with built-in Gemini) installed and authenticated — uses your standard account subscription, no API keys needed
